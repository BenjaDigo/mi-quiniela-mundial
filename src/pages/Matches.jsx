...
import { ChevronLeft, ChevronRight, RefreshCw, List, GitBranch } from 'lucide-react'
            ...
            <div className="flex bg-zinc-900 border border-zinc-800 rounded-xl p-1 gap-1">
              <button onClick={() => setView('list')}
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
                  view === 'list' ? 'bg-yellow-500 text-black' : 'text-zinc-400 hover:text-white'}`}>
                <List size={12} /> Lista
              </button>
              <button onClick={() => setView('bracket')}
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
                  view === 'bracket' ? 'bg-yellow-500 text-black' : 'text-zinc-400 hover:text-white'}`}>
                <GitBranch size={12} /> Cuadro
              </button>
            </div>
